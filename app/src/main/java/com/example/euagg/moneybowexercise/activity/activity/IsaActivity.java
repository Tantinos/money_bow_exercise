...
public class IsaActivity extends Activity {
    ...
    TextView isa_stock,isa_moneyBox;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        isa_stock = (TextView) findViewById(R.id.isa_stock);
        isa_moneyBox = (TextView)findViewById(R.id.isa_moneyBox);
        ...
        Gson gson = new Gson();
        final RequestInvest m = gson.fromJson(String.valueOf(jsonObject),RequestInvest.class);

        // Format integer to currency format
        format = NumberFormat.getCurrencyInstance(Locale.UK);
        String giaStock = format.format(m.getProducts().get(0).getPreviousMoneybox());
        String giaMoney = format.format(m.getProducts().get(0).getMoneybox());


        isa_stock.setText(giaStock);
        isa_moneyBox.setText(getResources().getString(R.string.your_moneybox)+giaMoney);
        investorProductId = m.getProducts().get(0).getInvestorProductId().toString();
    }

    // Method when user press the button to make a payment
    public void AddMoneyToISA(View view) throws IOException, InterruptedException {
    ...
    public void onBackPressed(){
        Intent intent = new Intent(IsaActivity.this, UserAccountActivity.class);
        intent.putExtra("BearerToken",bearer_token);
        startActivity(intent);
        finish();
    }

    public class AddMoney extends AsyncTask<Void,Void,Response>{
        ...
        protected void onPostExecute(Response result) {
            ...
            if(result.code()== 200){
                if(!(result == null)){
                    try{
                        ...
                        isa_moneyBox.setText(getResources().getString(R.string.your_moneybox)+value);
                    } catch (IOException e) {
                        e.printStackTrace();
                    } catch (JSONException e) {
                        e.printStackTrace();
            ...
            }else
            {
                ...
                new AlertDialog.Builder(IsaActivity.this)
                        ...
                        .setPositiveButton(getResources().getString(R.string.continue_),
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        Intent intent = new Intent(IsaActivity.this, LoginActivity.class);
